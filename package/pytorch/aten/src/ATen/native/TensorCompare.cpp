...
DEFINE_DISPATCH(protectclamp_stub); // NOLINT(cppcoreguidelines-avoid-non-const-global-variables)
...
Tensor& protectclamp_self_out(const Tensor& self, const Tensor& min, const Tensor& max, Tensor& out) {
...
Tensor protectclamp(const Tensor& self, const Tensor& min, const Tensor& max) {
  auto device = out_device(self, min, max);
  auto result_type = at::native::result_type(self);
  Tensor ret = at::empty({0}, self.options().dtype(result_type).device(device));
  at::native::protectclamp_self_out(self, min, max, ret);
  return ret;
}
