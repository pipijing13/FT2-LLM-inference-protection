...
void protectclamp_kernel_impl(TensorIteratorBase& iter) {
    //std::cout << "gpu" << std::endl;
    AT_DISPATCH_ALL_TYPES_AND2(kHalf, kBFloat16, iter.dtype(), "protectclamp_cuda", [&]() {
        gpu_kernel(iter, []GPU_LAMBDA(scalar_t v, scalar_t lower, scalar_t upper) -> scalar_t {
...
REGISTER_CUDA_DISPATCH(protectclamp_stub, &protectclamp_kernel_impl);
