...
from modeling_gptj_protected import GPTJForFICausalLM
# from transformers.models.gpt2.modeling_gpt2 import GPT2FlashAttention2
import numpy as np
import evaluate
import random
import pytorchfi
import pytorchfi.neuron_error_models as pyneuron
import pytorchfi.weight_error_models as pyweight
import pytorchfi.core as pycore
import torch
...
def hook1_fn(module, input, output):
    ...
    if flag == key_tokens[index]:
        ...
        position1 = random.randint(0, 31)
        # print(output.shape)
        x = random.randint(0, output.shape[1] - 1)
        # print(x)
        y = random.randint(0, output.shape[2] - 1)
        # print(y)
        bs = output[0, x, y].view(torch.int32)
        mask = 1 << position1
        bs = bs ^ mask
        output[0, x, y] = bs.view(torch.float32)

    if flag == key_tokens[index + 1]:
        #print(flag)
        #print(index+1)
        #print(key_tokens[index+1])
        position2 = random.randint(0, 31)
        x = random.randint(0, output.shape[1] - 1)
        # print(x)
        y = random.randint(0, output.shape[2] - 1)
        # print(y)
        bs = output[1, x, y].view(torch.int32)
        mask = 1 << position2
        bs = bs ^ mask
        output[1, x, y] = bs.view(torch.float32)
...
if __name__ == '__main__':
    ...
    model = GPTJForFICausalLM.from_pretrained("EleutherAI/gpt-j-6b", device_map="cuda")
    # model.to(device)
    print("model finish")
    ...
    index = 0
    global key_tokens
    with open("squadrandomtokengptj.txt", "r") as f:
        key_tokens = [int(line.strip()) for line in f]
    #print(key_tokens)

    global min1
    min1 = torch.arange(168, dtype=torch.float16).reshape(6, 28).fill_(1000)
    global max1
    max1 = torch.arange(168, dtype=torch.float16).reshape(6, 28).fill_(-1000)
    min1.to(device)
    max1.to(device)
    ...
    #    handles.append(handle2)
        ...
        #handle2 = model.transformer.h[i].attn.out_proj.register_forward_hook(hookop)
        ...
        #handle4 = model.transformer.h[i].attn.k_proj.register_forward_hook(hookkp)
        ...
        #handle6 = model.transformer.h[i].attn.q_proj.register_forward_hook(hookqp)
        ...
        #handle8 = model.transformer.h[i].attn.v_proj.register_forward_hook(hookvp)
        ...
        #handle10 = model.transformer.h[i].mlp.fc_in.register_forward_hook(hook1p)
        ...
        #handle12 = model.transformer.h[i].mlp.fc_out.register_forward_hook(hook2p)
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        for _ in range(20):
            ...
            if selected_layer == 'v_proj':
                handle1 = model.transformer.h[b].attn.v_proj.register_forward_hook(hook1_fn)
            elif selected_layer == 'k_proj':
                handle1 = model.transformer.h[b].attn.k_proj.register_forward_hook(hook1_fn)
            elif selected_layer == 'q_proj':
                handle1 = model.transformer.h[b].attn.q_proj.register_forward_hook(hook1_fn)
            elif selected_layer == 'out_proj':
                handle1 = model.transformer.h[b].attn.out_proj.register_forward_hook(hook1_fn)
            elif selected_layer == 'fc_in':
                handle1 = model.transformer.h[b].mlp.fc_in.register_forward_hook(hook1_fn)
            else:  # fc2
                handle1 = model.transformer.h[b].mlp.fc_out.register_forward_hook(hook1_fn)
            ...
            for i in range(28):
                ...
                #handle3 = model.transformer.h[i].attn.out_proj.register_forward_hook(hookoc)
                ...
                #handle5 = model.transformer.h[i].attn.k_proj.register_forward_hook(hookkc)
                ...
                #handle7 = model.transformer.h[i].attn.q_proj.register_forward_hook(hookqc)
                ...
                #handle9 = model.transformer.h[i].attn.v_proj.register_forward_hook(hookvc)
                ...
                #handle11 = model.transformer.h[i].mlp.fc_in.register_forward_hook(hook1c)
                ...
                #handle13 = model.transformer.h[i].mlp.fc_out.register_forward_hook(hook2c)
                #handles.append(handle2)
                #handles.append(handle3)
                #handles.append(handle4)
                #handles.append(handle5)
            ...
            for i in range(max_length):
                ...
                # 如果你想实现之前讨论的 [0, 1] pattern，可以这样修改：
                ...
                #    (
                #        torch.cat([layer_key[:, :, :-2, :], layer_key[:, :, -1:, :]], dim=2),  # 删除最后一个序列位置
                #        torch.cat([layer_value[:, :, :-2, :], layer_value[:, :, -1:, :]], dim=2)  # 删除最后一个序列位置
                #    )
