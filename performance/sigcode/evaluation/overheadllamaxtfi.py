...
if __name__ == '__main__':
    ...
    print(len(dataset))
    tokenizer = AutoTokenizer.from_pretrained("mgoin/llama2-7b-gsm8k-pt")
    print("before model")

    # model = OPTForCausalLM.from_pretrained("facebook/opt-2.7b", torch_dtype=torch.float16, attn_implementation="flash_attention_2")
    model = LlamaForFICausalLM.from_pretrained("mgoin/llama2-7b-gsm8k-pt", torch_dtype=torch.float16, device_map="cuda")
    # model.to(device)
    print("model finish")
    ...
    index = 0
    global key_tokens
    with open("xtremerandomtokenllama.txt", "r") as f:
