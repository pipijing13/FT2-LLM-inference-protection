...
def hook2_fn(module, input, output):
    ...
    if flag == key_tokens[index]:
        ...
        # print(y)
        bs = output[x, y].view(torch.int32)
        mask = 1 << position1
        bs = bs ^ mask
        output[x, y] = bs.view(torch.float32)
...
if __name__ == '__main__':
    ...
    model = OPTForFICausalLM.from_pretrained("facebook/opt-6.7b", device_map="cuda")
    # model.to(device)
    print("model finish")
    ...
    index = 0
    global key_tokens
    with open("squadrandomtokenopt.txt", "r") as f:
