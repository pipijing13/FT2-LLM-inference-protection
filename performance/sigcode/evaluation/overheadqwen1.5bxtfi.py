...
if __name__ == '__main__':
    ...
    print(len(dataset))
    tokenizer = AutoTokenizer.from_pretrained("Qwen/Qwen2-Math-1.5B")
    print("before model")

    # model = OPTForCausalLM.from_pretrained("facebook/opt-2.7b", torch_dtype=torch.float16, attn_implementation="flash_attention_2")
    model = Qwen2ForFICausalLM.from_pretrained("Qwen/Qwen2-Math-1.5B", torch_dtype=torch.float16, device_map="cuda")
    # model.to(device)
    print("model finish")
    ...
    index = 0
    global key_tokens
    with open("xtremerandomtokenqwen1.5b.txt", "r") as f:
