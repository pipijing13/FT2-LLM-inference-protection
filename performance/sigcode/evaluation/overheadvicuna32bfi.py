...
from modeling_llama_protected import LlamaForFICausalLM
...
if __name__ == '__main__':
    ...
    model = LlamaForFICausalLM.from_pretrained("lmsys/vicuna-7b-v1.5", device_map="cuda")
    # model.to(device)
    print("model finish")
    ...
    index = 0
    global key_tokens
    with open("squadrandomtokenvicuna.txt", "r") as f:
        key_tokens = [int(line.strip()) for line in f]
    #print(key_tokens)

    global min1
    min1 = torch.arange(224, dtype=torch.float16).reshape(7, 32).fill_(1000)
    global max1
    max1 = torch.arange(224, dtype=torch.float16).reshape(7, 32).fill_(-1000)
    min1.to(device)
    max1.to(device)
