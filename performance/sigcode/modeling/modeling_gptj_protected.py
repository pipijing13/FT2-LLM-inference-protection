...
class GPTJAttention(nn.Module):
    def __init__(self, config):
        ...
        self.register_buffer('min_v_proj', torch.tensor(float('inf')))
        self.register_buffer('max_v_proj', torch.tensor(float('-inf')))
        self.register_buffer('min_out_proj', torch.tensor(float('inf')))
        self.register_buffer('max_out_proj', torch.tensor(float('-inf')))
    ...
    def forward(
        ...
        if not calibration_mode:
            ...
            # print(self.min_v_proj.device)
            # value_states = self.clamp_to_zero(value_states, self.min_v_proj.item(), self.max_v_proj.item())
            # value_states = self.clamp_to_zero(value_states, self.min_v_proj, self.max_v_proj)
            value = torch.protectclamp(value, min=self.min_v_proj, max=self.max_v_proj)
            # value_states = torch.nan_to_num(value_states, nan=0.0)
        else:
            self.min_v_proj = value.min() * 2
            self.max_v_proj = value.max() * 2
        ...
        if not calibration_mode:
            #attn_output = self.clamp_to_zero(attn_output, self.min_out_proj.item(), self.max_out_proj.item())
            #attn_output = self.clamp_to_zero(attn_output, self.min_out_proj, self.max_out_proj)
            attn_output = torch.protectclamp(attn_output, min=self.min_out_proj, max=self.max_out_proj)
            #print(self.min_out_proj)
            #attn_output = torch.nan_to_num(attn_output, nan=0.0)
        else:
            self.min_out_proj = attn_output.min()*2
            #print(self.min_out_proj)
            self.max_out_proj = attn_output.max()*2
...
class GPTJMLP(nn.Module):
    def __init__(self, intermediate_size, config):  # in MLP: intermediate_size= 4 * embed_dim
        ...
        self.register_buffer('min_fc2', torch.tensor(float('inf')))
        self.register_buffer('max_fc2', torch.tensor(float('-inf')))

    def forward(self, hidden_states: Optional[torch.FloatTensor], calibration_mode: bool = True) -> torch.FloatTensor:
        ...
        if not calibration_mode:
            ...
            hidden_states = torch.protectclamp(hidden_states, min=self.min_fc2, max=self.max_fc2)
            #hidden_states = torch.nan_to_num(hidden_states, nan=0.0)
        else:
            self.min_fc2 = hidden_states.min()*2
            self.max_fc2 = hidden_states.max()*2
...
class GPTJBlock(nn.Module):
    ...
    def forward(
        ...
        feed_forward_hidden_states = self.mlp(hidden_states, calibration_mode=calibration_mode)
...
class GPTJForFICausalLM(GPTJPreTrainedModel):
